  ...
  return (
    <nav onScroll={e => changeNavBG(e)} className={`${navBG} backdrop-blur-xl fixed w-full z-30 top-0 start-0 border border-b-main-strong`}>
      <div className="max-w-7xl flex flex-wrap items-center justify-between mx-auto p-4">
        <NavLink to={"/"} className="flex items-center group/navbar space-x-3">
        ...
        <div className="flex items-center md:order-2 space-x-3 md:space-x-0">
          <NavLink to={"/blog"}><button type="button" className="cursor-pointer hidden text-white  hover:text-main-strong me-2 box-border border border-transparent hover:border-gray-600 shadow-xs font-medium leading-5 rounded-xl text-md self-center size-10 md:block transition"><i className="fa-solid fa-search"></i></button></NavLink>
        ...
        <div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-sticky">
          <ul className="flex flex-col max-md:text-start text-center p-4 font-medium border! border-secondary rounded-xl max-md:mt-5 md:rounded-full md:p-1.5  md:space-x-8 md:flex-row md:mt-0 md:border-0   md:gap-0">
            <li className='md:m-0'>
              <NavLink onClick={navCollapse} to={"/home" && "/"} className={({ isActive }) => (isActive ? "active-link link" : "link")} aria-current="page">الرئيسية</NavLink>
            </li>
            <li className='md:m-0'>
              <NavLink onClick={navCollapse} to={"/blog"} className={({ isActive }) => (isActive ? "active-link link" : "link")}>المدونة</NavLink>
            </li>
            <li className='md:m-0'>
              <NavLink onClick={navCollapse} to={"/about"} className={({ isActive }) => (isActive ? "active-link link" : "link")}>من نحن</NavLink>
            </li>
            <li>
              <NavLink onClick={navCollapse} to={'/blog'}><button type="button" className="bg-main cursor-pointer block md:hidden mt-2 w-full text-white hover:text-gray-200 hover:-translate-y-1 box-border shadow-xs font-medium leading-5 rounded-full text-lg py-4 px-8 transition-all">إبدأ القراءة</button></NavLink>
            </li>
          </ul>
        </div>
      </div>
