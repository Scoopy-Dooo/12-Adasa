...
export default function Posts({ postsIDs, category }) {
    ...
    // myCategories = myCategories.filter((cate) => cate.name === type)

    // change posts date from numeric to string formula & split content
    posts.map((post) => {
        // content spliter
        post.content = (post.content.split("##"));
        post.content = post.content.map((item) => item = item.split("\n\n"));
        // date change
        post.date = new Date(post.date).toLocaleDateString('ar-EG', { year: 'numeric', month: 'long', day: 'numeric' });
        return post
    })

    // get wanted posts from posts array
    let wantedPosts;
    if (category) {
        if (category == "all") {
            wantedPosts = posts
        } else {
            wantedPosts = posts.filter((post) => {
